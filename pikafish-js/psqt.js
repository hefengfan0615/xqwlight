...
let psq = new Array(T.PIECE_NB);

const S = T.make_score;

// Bonus tables for each piece type (Rook, Advisor, Cannon, Pawn, Knight, Bishop, King)
const Bonus = [
  [], // No piece type
  [ // ROOK
...
function edge_distance(f) {
  return Math.min(f, T.FILE_I - f);
}

// Initialize PSQT
function init() {
  for (let pc = 0; pc < T.PIECE_NB; pc++) {
    psq[pc] = new Array(T.SQUARE_NB).fill(0);
  }
  
  // White pieces
  const whitePieces = [T.W_ROOK, T.W_ADVISOR, T.W_CANNON, T.W_PAWN, T.W_KNIGHT, T.W_BISHOP, T.W_KING];
  
  for (const pc of whitePieces) {
    const pt = T.type_of(pc);
    const score = T.make_score(T.PieceValue[T.MG][pc], T.PieceValue[T.EG][pc]);
    
    for (let s = T.SQ_A0; s <= T.SQ_I9; s++) {
      const f = edge_distance(T.file_of(s));
      const adjustedF = f > T.FILE_E ? T.FILE_E * 2 - f : f;
      
      psq[pc][s] = score + Bonus[pt][T.rank_of(s)][adjustedF];
      psq[pc + 8][T.flip_rank(s)] = -psq[pc][s];
    }
  }
}

// Get PSQT score for a piece on a square
function psq_score(pc, sq) {
  return psq[pc][sq];
}
